...
const Eg18GUIMore = () => {
  ...
  useEffect(() => {
    ...
    gui.add(guiChangeData, 'x', 0, 180, 1).name('网格模型位置X').onChange((val) => {
      mesh.position.x = val;
    });

    gui.add(guiChangeData, 'scale', [-100, 0, 100]).name('网格模型y坐标').onChange((val) => {
      mesh.position.y = val;
    });

    gui.add(guiChangeData, 'scale', {
      left: -100,
      center: 0,
      right: 100
    }).name('选择位置').onChange((val) => {
      mesh.position.x = val;
    });

    gui.add(guiChangeData, 'bool').name('网格模型旋转');
    ...
    const render = () => {
      if (guiChangeData.bool) {
        mesh.rotateY(0.01); // 每帧旋转0.01弧度
      }
