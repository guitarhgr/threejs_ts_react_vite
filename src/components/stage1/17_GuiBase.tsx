...
const Eg17GUIBase = () => {
  ...
  useEffect(() => {
    ...
    // gui控制网格模型位置
    gui.add(mesh.position, 'x', 0, 180, 1).name('网格模型位置X');
    gui.add(mesh.position, 'y', 0, 180, 1).name('网格模型位置X');
    gui.add(mesh.position, 'z', 0, 180, 1).name('网格模型位置X');

    const meshChangeObj = {
      color: '#ff0000', // 默认颜色
    };
    gui.addColor(meshChangeObj, 'color').name('网格模型颜色').onChange((val) => {
    ...
    // gui控制平行光强度
    gui.add(ambientLight, 'intensity', 0, 2.0, 0.01).name('环境光强度');
